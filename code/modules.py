...
class BiDAF(object):
    ...
    def build_graph(self, q, q_mask, c, c_mask):
        ...
        with vs.variable_scope("BiDAF"):
            ...
            _, c2q_dist = masked_softmax(similarity, similarity_mask,
                                         2)  # shape (batch_size, N, M). take softmax over q
            print(c2q_dist)
...
I       Implement question passage matching from R-Net
        ...
        for i in range(context_len):
            ...
            concat_u_Pi = tf.concat(
                [tf.reshape(u_P[:, i, :], [cur_batch_size, 1, 2 * self.hidden_size_encoder])] * question_len, 1)
            ...
            if i == 0:
                ...
                concat_v_Pi = tf.concat(
                    [tf.reshape(v_P[i - 1], [cur_batch_size, 1, self.hidden_size_qp])] * question_len, 1)
    ...
    def build_graph_answer_pointer(self, context_encoding, question_encoding, values_mask, context_mask, context_len,
        ...
        for i in range(2):
            ...
            concat_h_i1a = tf.concat(
                [tf.reshape(h_i1a, [cur_batch_size, 1, 2 * self.hidden_size_encoder])] * context_len, 1)
