...
from official_eval_helper import get_json_data, generate_answers, generate_answers_attention
...
tf.app.flags.DEFINE_integer("char_out_size", 100,
...
def main(unused_argv):
    ...
    if FLAGS.mode == "train":
        ...
        with tf.Session(config=config) as sess:
            ...
            # Get a predicted answer for each example in the data
            # Return a mapping answers_dict from uuid to answer
            answers_dict = generate_answers_attention(sess, qa_model, word2id, qn_uuid_data, context_token_data,
                                                      qn_token_data)
